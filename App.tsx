...
import ContactModal from './components/PdfModal';
...
const App: React.FC = () => {
  ...
  const [contactButtonBounds, setContactButtonBounds] = useState<DOMRect | null>(null);
    ...
    const fetchMajorProjects = async () => {
      try {
        ...
        const projectPromises = projectIds.map(id => 
          fetch(`./ProjectData/${id}.json`).then(res => res.json())
        );
        const projects = await Promise.all(projectPromises);
        setMajorProjects(projects);
      } catch (error) {
        console.error("Failed to load major projects:", error);
      } finally {
  ...
  const handleOpenContactModal = (element: HTMLElement) => {
    setIsContactModalClosing(false);
    setContactButtonBounds(element.getBoundingClientRect());
    setIsContactModalOpen(true);
  };

  const handleCloseContactModal = () => {
      setIsContactModalOpen(false);
      setContactButtonBounds(null);
  };

  const handleStartContactFalling = () => {
      setIsContactModalClosing(true);
  };

  const isBackdropVisible = (selectedProject !== null && !isModalClosing) || (selectedDocument !== null && !isDocumentModalClosing) || (isContactModalOpen && !isContactModalClosing);
  ...
  return (
    <OpeningAnimation>
      <div className="min-h-screen">
        ...
        {isContactModalOpen && contactButtonBounds && (
          <ContactModal
              initialBounds={contactButtonBounds}
              onClose={handleCloseContactModal}
              onStartFalling={handleStartContactFalling}
          />
        )}
        
        <Header onContactClick={handleOpenContactModal} isContactModalOpen={isContactModalOpen} />
        <main>
